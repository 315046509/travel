class Admin::TejiaController < Admin::MainController
  before_action :set_get_resource, :only => [:destroy]
  layout 'admin'

  def index
    @lunbos = Tejia.where(:city_id => params[:city_id]).order("id desc").page(params[:page]).per(10)
  end
    ...
    if request.post?
      if params[:tejia]
        ...
        if !avatar.blank? && !title.blank? && !description.blank? && !price.blank?
          ...
          redirect_to admin_city_tejia_path(params[:city_id]) and return
        else
  ...
  # delete
  def destroy
    @resource = Tejia.find(params[:id])
    @resource.destroy
    respond_to do |r|
      r.html do
        redirect_to admin_city_tejia_path(@resource.city_id)
        return
      end
    end
  end
